...
| GET | `/api/uploadingVideos?channel_id=` | Returns `{ uploads }` — **all** of the channel's user-uploaded videos (completed, pending, failed), newest first. Identified by `upload_status != 0` or a Cloudinary/local `/uploads/` link. Includes `thumbnail_link`, `video_description`, `tags`, `category`, `isShort` so rows can be edited. |
| POST | `/api/updateVideo` | Edits a video's metadata only (no file re-upload): `video_id`, `user_id` (channel_id), `title`, `description`, `tags`, `category`, `isShort` (`"video"`/`"short"`/0/1), `thumbnail_link` (set to a new `/api/upload` URL to change it). `UPDATE` is scoped to `video_id AND channel_id = user_id`, returning 403 if the video isn't owned by that channel. |
